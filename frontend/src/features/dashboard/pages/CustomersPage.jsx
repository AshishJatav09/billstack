...
const CustomersPage = () => {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-6 pb-8">
    ...
    <div className="grid gap-6 xl:grid-cols-[330px_minmax(0,1fr)]"><aside className="h-fit rounded-2xl border" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="border-b p-4" style={{ borderColor: "var(--panel-border)" }}><label className="relative block"><Search size={16} className="absolute left-3 top-3" style={{ color: "var(--text-muted)" }} /><input name="search" value={filters.search} onChange={changeFilter} placeholder="Search customers" className="w-full rounded-xl border bg-transparent py-2.5 pl-9 pr-3 text-sm" style={{ borderColor: "var(--panel-border)" }} /></label></div>{loading ? <div className="p-4"><LoadingState title="Loading customers" description="Fetching customer records." /></div> : result.items.length ? <div className="max-h-[650px] overflow-y-auto">{result.items.map((customer) => <button key={customer._id} onClick={() => { setSelectedId(customer._id); setTab("overview"); }} className="w-full border-b p-4 text-left hover:bg-slate-500/[.04]" style={{ borderColor: "var(--panel-border)", background: selected?._id === customer._id ? "color-mix(in srgb, var(--accent) 7%, transparent)" : "transparent" }}><p className="font-semibold">{customer.name}</p><p className="mt-1 truncate text-xs" style={{ color: "var(--text-muted)" }}>{customer.email || customer.phone || "No contact details"}</p><p className="mt-2 text-xs text-brand-600">Open financial workspace</p></button>)}</div> : <div className="p-4"><EmptyState title="No customers found" description="Create a customer to start billing." /></div>}</aside>
      <main className="min-w-0 space-y-6">{selected ? <><section className="rounded-2xl border p-5 sm:p-6" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between"><div><p className="text-sm font-medium text-brand-600">Customer financial workspace</p><h3 className="mt-1 text-2xl font-semibold">{selected.name}</h3><p className="mt-2 text-sm" style={{ color: "var(--text-muted)" }}>{[selected.email, selected.phone].filter(Boolean).join(" · ") || "No contact details recorded"}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => { setPaymentForm(blankPayment()); setPaymentErrors({}); setPaymentOpen(true); }} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white"><WalletCards size={16} /> Receive payment</button><button onClick={() => { setCustomerForm({ ...blankCustomer, ...selected }); setEditor("edit"); }} className="rounded-xl border px-4 py-2.5 text-sm" style={{ borderColor: "var(--panel-border)" }}>Edit customer</button><button onClick={deleteCustomer} className="rounded-xl border border-rose-500/30 px-4 py-2.5 text-sm text-rose-600">Delete</button></div></div><div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Outstanding", metrics.outstanding], ["Overdue", metrics.overdue], ["Total invoiced", metrics.invoiced], ["Total collected", metrics.collected]].map(([label, value]) => <div key={label} className="rounded-xl border p-4" style={{ borderColor: "var(--panel-border)" }}><p className="text-xs font-medium uppercase tracking-wide" style={{ color: "var(--text-muted)" }}>{label}</p><p className="mt-2 text-xl font-semibold">{money(value)}</p></div>)}</div></section>
        <nav className="flex overflow-x-auto rounded-xl border p-1" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}>{["overview", "invoices", "payments", "ledger", "statement", "communications"].map((item) => <button key={item} onClick={() => setTab(item)} className="whitespace-nowrap rounded-lg px-4 py-2.5 text-sm font-medium capitalize" style={tab === item ? { background: "var(--accent)", color: "white" } : { color: "var(--text-muted)" }}>{item}</button>)}</nav>
