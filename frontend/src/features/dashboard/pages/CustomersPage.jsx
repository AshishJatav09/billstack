...
const CustomersPage = () => {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-6 pb-8">
    ...
    <div className="grid gap-6 xl:grid-cols-[310px_minmax(0,1fr)]"><aside className="h-fit rounded-2xl border" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="border-b p-3" style={{ borderColor: "var(--panel-border)" }}><label className="relative block"><Search size={16} className="absolute left-3 top-3" style={{ color: "var(--text-muted)" }} /><input name="search" value={filters.search} onChange={changeFilter} placeholder="Search customers" className="w-full rounded-xl border bg-transparent py-2.5 pl-9 pr-3 text-sm" style={{ borderColor: "var(--panel-border)" }} /></label></div>{loading ? <div className="p-4"><LoadingState title="Loading customers" description="Fetching customer records." /></div> : result.items.length ? <div className="max-h-[650px] overflow-y-auto no-scrollbar">{result.items.map((customer) => <button key={customer._id} onClick={() => { setSelectedId(customer._id); setTab("overview"); }} className="w-full border-b px-4 py-3 text-left transition hover:bg-slate-500/[.04]" style={{ borderColor: "var(--panel-border)", background: selected?._id === customer._id ? "color-mix(in srgb, var(--accent) 7%, transparent)" : "transparent" }}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{customer.name}</p><p className="mt-0.5 truncate text-xs" style={{ color: "var(--text-muted)" }}>{customer.email || customer.phone || "No contact details"}</p></div><span className="shrink-0 text-lg leading-none text-brand-600">�</span></div></button>)}</div> : <div className="p-4"><EmptyState title="No customers found" description="Create a customer to start billing." /></div>}</aside>
