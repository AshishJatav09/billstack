...
const CustomersPage = () => {
  ...
  const saveCustomer = async (event) => { event.preventDefault(); if (saveCustomerLock.current) return; saveCustomerLock.current = true; setSavingCustomer(true); try { if (editor === "edit") await updateCustomerRequest(selected._id, customerForm); else await createCustomerRequest(customerForm); uiStore.getState().pushToast({ tone: "success", message: editor === "edit" ? "Customer updated successfully." : "Customer created successfully." }); setEditor(null); await loadCustomers(); } catch (err) { setError(err.response?.data?.message || "Unable to save customer."); } finally { saveCustomerLock.current = false; setSavingCustomer(false); } };
  ...
  return <div className="mx-auto max-w-[1600px] space-y-6 pb-8">
    ...
    <div className="grid gap-6"><aside className="rounded-2xl border p-3" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="grid gap-3 lg:grid-cols-[1fr_320px]"><label className="relative block"><Search size={16} className="absolute left-3 top-3" style={{ color: "var(--text-muted)" }} /><input name="search" value={filters.search} onChange={changeFilter} placeholder="Search customers" className="w-full rounded-xl border bg-transparent py-2.5 pl-9 pr-3 text-sm" style={{ borderColor: "var(--panel-border)" }} /></label>{loading ? <div className="rounded-xl border px-3 py-2.5 text-sm" style={{ borderColor: "var(--panel-border)", color: "var(--text-muted)" }}>Loading customers...</div> : result.items.length ? <select value={selected?._id || ""} onChange={(event) => { setSelectedId(event.target.value); setTab("overview"); }} className="field"><option value="">Select customer</option>{result.items.map((customer) => <option key={customer._id} value={customer._id}>{customer.name}{customer.email ? " · " + customer.email : customer.phone ? " · " + customer.phone : ""}</option>)}</select> : <button type="button" onClick={() => { setCustomerForm(blankCustomer); setEditor("new"); }} className="rounded-xl border px-3 py-2.5 text-left text-sm" style={{ borderColor: "var(--panel-border)" }}>No customers found. Add customer</button>}</div>{selected ? <div className="mt-3 rounded-xl border px-4 py-3" style={{ borderColor: "var(--panel-border)", background: "color-mix(in srgb, var(--accent) 6%, transparent)" }}><p className="text-sm font-semibold">{selected.name}</p><p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>{selected.email || selected.phone || "No contact details"}</p></div> : null}</aside>
...
const CustomerModal = ({ clientWorkspace, form, setForm, mode, saving, error, onSave, onClose }) => <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-4"><form id="customer-editor" onSubmit={onSave} className="mx-auto my-5 w-full max-w-2xl rounded-2xl border p-6 shadow-2xl" style={{ borderColor: "var(--panel-border)", background: "var(--theme-surface-strong)" }}><div className="flex justify-between"><div><p className="text-sm font-medium text-brand-600">Customer details</p><h3 className="mt-1 text-xl font-semibold">{mode === "edit" ? clientWorkspace ? "Edit client" : "Edit customer" : clientWorkspace ? "Add client" : "Add customer"}</h3></div><button type="button" onClick={onClose}><X size={20} /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{[["name", "Name"], ["email", "Email"], ["phone", "Phone"], ["gstNumber", "GSTIN"], ["billingAddress", "Billing address"], ["shippingAddress", "Shipping address"]].map(([key, label]) => <Field key={key} label={label}><input required={key === "name"} value={form[key] || ""} onChange={(event) => setForm((v) => ({ ...v, [key]: event.target.value }))} className="field" /></Field>)}</div><Field label="Notes"><textarea rows="3" value={form.notes || ""} onChange={(event) => setForm((v) => ({ ...v, notes: event.target.value }))} className="field" /></Field><p role="alert" className="mt-3 text-sm text-rose-600">{error}</p><div className="mt-6 flex justify-end gap-3"><button type="button" disabled={saving} onClick={onClose} className="rounded-xl border px-4 py-2.5 text-sm" style={{ borderColor: "var(--panel-border)" }}>Cancel</button><button disabled={saving} className="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{saving ? "Saving..." : "Save customer"}</button></div></form></div>;
