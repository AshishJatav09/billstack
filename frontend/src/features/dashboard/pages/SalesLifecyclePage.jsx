...
const SalesLifecyclePage = () => {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4 pb-6">
    <section className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}>
    ...
    {tab === "quotes" ? <section className="space-y-4"><QuoteEditor form={quoteForm} setForm={setQuoteForm} editing={editingQuote} setEditing={setEditingQuote} products={products} customers={customers} saving={saving === "quote"} onSubmit={saveQuote} /><QuoteList rows={quotes} saving={saving} onView={setSelectedQuote} onEdit={editQuote} onTransition={transitionQuote} onConvert={convertQuote} onSend={sendQuote} onDownload={downloadQuote} /></section> : null}
...
const QuoteEditor = ({ form, setForm, editing, setEditing, products, customers, saving, onSubmit }) => {
  ...
  return <form id="quote-editor" onSubmit={onSubmit} className="rounded-2xl border p-4 sm:p-5" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">{editing ? "Edit draft quote" : "Create quote"}</h3><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>Select a client and items to prepare your quotation.</p></div>{editing ? <button type="button" onClick={() => { setEditing(null); setForm(blankQuoteForm()); }}><X size={18} /></button> : null}</div><div className="mt-4 grid gap-3"><Field label="Customer"><select required value={form.customerId} onChange={(event) => setForm((value) => ({ ...value, customerId: event.target.value }))} className="field py-2"><option value="">Select customer</option>{customers.map((customer) => <option key={customer._id} value={customer._id}>{customer.name}</option>)}</select></Field><GstLocationPreview form={form} setForm={setForm} customers={customers} /><div className="space-y-3">{form.lineItems.map((line, index) => <div key={index} className="rounded-xl border p-3" style={{ borderColor: "var(--panel-border)" }}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(180px,2fr)_repeat(5,minmax(0,1fr))]"><Field label="Item / service"><select required value={line.productId} onChange={(event) => { const product = products.find((item) => item._id === event.target.value); updateLine(index, { productId: event.target.value, rate: product?.sellingPrice ?? line.rate, taxRate: product?.taxRate ?? line.taxRate }); }} className="field py-2"><option value="">Select item/service</option>{products.map((product) => <option key={product._id} value={product._id}>{product.name}</option>)}</select></Field><Field label="Quantity"><input required type="number" min="0.01" step="0.01" value={line.quantity} onChange={(event) => updateLine(index, { quantity: event.target.value })} className="field py-2" /></Field><Field label="Rate"><input type="number" min="0" step="0.01" value={line.rate} onChange={(event) => updateLine(index, { rate: event.target.value })} className="field py-2" /></Field><Field label="GST %"><input type="number" min="0" step="0.01" value={line.taxRate} onChange={(event) => updateLine(index, { taxRate: event.target.value })} className="field py-2" /></Field><Field label="Discount"><input type="number" min="0" step="0.01" value={line.discountValue} onChange={(event) => updateLine(index, { discountValue: event.target.value })} className="field py-2" /></Field><Field label="Discount type"><select value={line.discountType} onChange={(event) => updateLine(index, { discountType: event.target.value })} className="field py-2"><option value="percent">Percent</option><option value="amount">Amount</option></select></Field></div>{form.lineItems.length > 1 ? <button type="button" onClick={() => removeLine(index)} className="mt-3 text-sm text-rose-600">Remove line</button> : null}</div>)}</div><div className="flex flex-wrap items-center gap-3 border-t pt-3" style={{ borderColor: "var(--panel-border)" }}><button type="button" onClick={addLine} className="rounded-xl border px-4 py-2.5 text-sm font-medium" style={{ borderColor: "var(--panel-border)" }}>Add line</button><p className="min-w-0 flex-1 text-sm font-semibold">Estimated total: {money(estimatedQuoteTotal(form))}</p><button type="button" disabled={saving} onClick={() => { setEditing(null); setForm(blankQuoteForm()); }} className="rounded-xl border px-4 py-2 text-sm">Cancel</button><button disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-60"><Send size={16} /> {saving ? "Saving..." : editing ? "Update draft quote" : "Create quote"}</button></div></div></form>;
};
