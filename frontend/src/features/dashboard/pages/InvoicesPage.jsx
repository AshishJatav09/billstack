...
const InvoicesPage = () => {
  ...
  const save = async (event, { send = false, recordPaymentAfterIssue = false } = {}) => {
    ...
    try {
      ...
      const upfrontAmount = form.paymentMode === "full" ? totals.grand : Number(form.upfrontPaymentAmount || 0);
      if (!editingId && form.paymentMode !== "unpaid") {
        await createAndAllocatePayment(invoice, upfrontAmount, {
  ...
  const createAndAllocatePayment = async (invoice, amount, paymentFields = paymentForm) => {
    const freshInvoice = await getInvoiceRequest(invoice._id);
    const authoritativeInvoice = freshInvoice?.invoice || freshInvoice?.data || freshInvoice;
    const outstanding = Number(authoritativeInvoice?.balanceDue || 0);
    if (outstanding <= 0) return { alreadyPaid: true, invoice: authoritativeInvoice };
    const safeAmount = Math.min(Number(amount || 0), outstanding);
    if (!Number.isFinite(safeAmount) || safeAmount <= 0) throw new Error("Enter an amount greater than zero.");
    ...
    const payment = await createPaymentRequest({
      ...
      customerId,
      currency: "INR",
      paymentMethod: paymentFields.paymentMethod || "BANK_TRANSFER",
      paymentDate: paymentFields.paymentDate || today(),
      referenceNumber: paymentFields.referenceNumber || "",
      notes: paymentFields.notes || "",
      idempotencyKey: paymentFields.idempotencyKey || crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`,
    });
    await allocatePaymentRequest(payment._id, { invoiceId: authoritativeInvoice._id || invoice._id, allocatedAmount: safeAmount });
    return { alreadyPaid: false, invoice: authoritativeInvoice, amount: safeAmount };
  };
  ...
  const renderActionMenu = () => {
    ...
    return createPortal(
      <div data-invoice-menu className="fixed z-[70] max-h-[min(320px,calc(100vh-24px))] w-52 overflow-y-auto rounded-xl border p-1 text-left shadow-2xl no-scrollbar" style={{ top: Math.max(menuPosition.top, 12), right: menuPosition.right, borderColor: "var(--panel-border)", background: "var(--theme-surface-strong)" }}>
        <button type="button" onClick={() => edit(invoice)} disabled={busy || invoice.status === "cancelled"} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-slate-500/10 disabled:opacity-40">{active.type === "edit" ? <Loader2 size={15} className="animate-spin" /> : <Pencil size={15} />} Edit</button>
        {invoice.status !== "cancelled" && Number(invoice.balanceDue || 0) > 0 ? <button type="button" onClick={() => openPaymentModal(invoice)} disabled={busy} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-slate-500/10 disabled:opacity-40">{active.type === "payment" ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />} Record Payment</button> : null}
