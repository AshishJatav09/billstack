...
const makePaymentForm = () => ({ amount: "", paymentMethod: "BANK_TRANSFER", paymentDate: today(), referenceNumber: "", notes: "" });
...
const InvoicesPage = () => {
  ...
  const save = async (event, { send = false } = {}) => {
  ...
  const recordPayment = async (event) => {
    event.preventDefault();
    if (!postIssue?.invoice || paymentSaving) return;
    ...
    try {
      const customerId = postIssue.invoice.customerId?._id || postIssue.invoice.customerId;
      const payment = await createPaymentRequest({ amount, direction: "RECEIVED", customerId, currency: "INR", paymentMethod: paymentForm.paymentMethod, paymentDate: paymentForm.paymentDate, referenceNumber: paymentForm.referenceNumber, notes: paymentForm.notes });
      await allocatePaymentRequest(payment._id, { invoiceId: postIssue.invoice._id, allocatedAmount: amount });
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-8">
    ...
    {showEditor ? <form onSubmit={(event) => save(event)} className="rounded-2xl border p-5 sm:p-6" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="flex flex-wrap items-start justify-between gap-4 border-b pb-5" style={{ borderColor: "var(--panel-border)" }}><div><p className="text-sm font-medium text-brand-600 dark:text-brand-300">{editingId ? "Editing invoice" : "New invoice"}</p><h3 className="mt-1 text-xl font-semibold">{editingId ? "Update invoice details" : "Create an invoice"}</h3><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>Normal creation issues the invoice immediately. True draft support stays hidden until ledger-safe draft semantics exist.</p></div><button type="button" onClick={resetEditor} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm" style={{ borderColor: "var(--panel-border)" }}><X size={16} /> Close</button></div>
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"><div className="space-y-6"><section><div className="flex items-center justify-between gap-3"><h4 className="font-semibold">Customer</h4><button type="button" onClick={() => setCustomerOpen(true)} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium" style={{ borderColor: "var(--panel-border)" }}><Plus size={16} /> Add new customer</button></div><label className="mt-4 block"><span className="mb-2 block text-sm font-medium">Search/select customer</span><div className="relative"><Search size={16} className="pointer-events-none absolute left-3 top-3.5" style={{ color: "var(--text-muted)" }} /><select name="customerId" value={form.customerId} onChange={changeForm} className="w-full rounded-xl border bg-transparent py-3 pl-9 pr-3 text-sm" style={{ borderColor: "var(--panel-border)" }}><option value="">Select customer</option>{customers.map((customer) => <option key={customer._id} value={customer._id}>{customer.name}{customer.phone ? ` · ${customer.phone}` : ""}</option>)}</select></div>{errors.customerId ? <span className="mt-1 block text-xs text-rose-500">{errors.customerId}</span> : null}</label>{selectedCustomer ? <div className="mt-3 rounded-xl bg-brand-500/5 px-4 py-3 text-sm"><p className="font-medium">{selectedCustomer.name}</p><p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>{[selectedCustomer.email, selectedCustomer.phone, selectedCustomer.gstNumber, selectedCustomer.stateCode].filter(Boolean).join(" · ") || "No contact details recorded"}</p></div> : null}</section>
        ...
        <section className="border-t pt-6" style={{ borderColor: "var(--panel-border)" }}><div className="flex items-center justify-between gap-4"><div><h4 className="font-semibold">Items</h4><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>Select a product/service or type a one-off manual item/service.</p></div><button type="button" onClick={() => setForm((current) => ({ ...current, lineItems: [...current.lineItems, makeLine()] }))} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium" style={{ borderColor: "var(--panel-border)" }}><Plus size={16} /> Add item/service</button></div><div className="mt-4 overflow-x-auto rounded-xl border" style={{ borderColor: "var(--panel-border)" }}><table className="min-w-[1100px] w-full text-left text-sm"><thead className="bg-slate-500/5 text-xs uppercase tracking-wide" style={{ color: "var(--text-muted)" }}><tr><th className="p-3">Master item</th><th className="p-3">Description</th><th className="p-3">Qty</th><th className="p-3">Rate</th><th className="p-3">Discount</th><th className="p-3">GST %</th><th className="p-3">HSN/SAC</th><th className="p-3">Save</th><th className="p-3 text-right">Amount</th><th className="p-3" /></tr></thead><tbody>{form.lineItems.map((item, index) => <tr key={index} className="border-t align-top" style={{ borderColor: "var(--panel-border)" }}><td className="p-2"><select value={item.productId} onChange={(event) => changeLine(index, "productId", event.target.value)} className="w-52 rounded-lg border bg-transparent px-2.5 py-2" style={{ borderColor: "var(--panel-border)" }}><option value="">Manual / one-off</option>{products.map((product) => <option key={product._id} value={product._id}>{product.name}{product.trackInventory ? ` · stock ${product.currentStock}` : " · service"}</option>)}</select></td><td className="p-2"><input value={item.productName} disabled={Boolean(item.productId)} onChange={(event) => changeLine(index, "productName", event.target.value)} placeholder="Type item or service" className="w-56 rounded-lg border bg-transparent px-2.5 py-2 disabled:opacity-70" style={{ borderColor: "var(--panel-border)" }} /></td><td className="p-2"><input value={item.quantity} onChange={(event) => changeLine(index, "quantity", event.target.value)} className="w-20 rounded-lg border bg-transparent px-2.5 py-2" style={{ borderColor: "var(--panel-border)" }} /></td><td className="p-2"><input value={item.rate} onChange={(event) => changeLine(index, "rate", event.target.value)} className="w-28 rounded-lg border bg-transparent px-2.5 py-2" style={{ borderColor: "var(--panel-border)" }} /></td><td className="p-2"><div className="flex"><input value={item.discountValue} onChange={(event) => changeLine(index, "discountValue", event.target.value)} className="w-20 rounded-l-lg border bg-transparent px-2.5 py-2" style={{ borderColor: "var(--panel-border)" }} /><select value={item.discountType} onChange={(event) => changeLine(index, "discountType", event.target.value)} className="rounded-r-lg border border-l-0 bg-transparent px-1 py-2 text-xs" style={{ borderColor: "var(--panel-border)" }}><option value="percent">%</option><option value="amount">INR</option></select></div></td><td className="p-2"><input value={item.taxRate} onChange={(event) => changeLine(index, "taxRate", event.target.value)} className="w-20 rounded-lg border bg-transparent px-2.5 py-2" style={{ borderColor: "var(--panel-border)" }} /></td><td className="p-2"><input value={item.hsnSac} disabled={Boolean(item.productId)} onChange={(event) => changeLine(index, "hsnSac", event.target.value)} className="w-24 rounded-lg border bg-transparent px-2.5 py-2 disabled:opacity-70" style={{ borderColor: "var(--panel-border)" }} /></td><td className="p-2"><label className="flex items-center gap-2 text-xs" style={{ color: "var(--text-muted)" }}><input type="checkbox" checked={!item.productId && item.saveForFuture} disabled={Boolean(item.productId)} onChange={(event) => changeLine(index, "saveForFuture", event.target.checked)} /> For future</label></td><td className="p-2 text-right font-medium">{money(totals.items[index]?.total)}</td><td className="p-2 text-right"><button type="button" disabled={form.lineItems.length === 1} onClick={() => setForm((current) => ({ ...current, lineItems: current.lineItems.filter((_, position) => position !== index) }))} className="rounded-lg p-2 text-rose-500 disabled:opacity-30"><Trash2 size={16} /></button></td></tr>)}</tbody></table></div>{errors.lineItems ? <p className="mt-2 text-xs text-rose-500">{errors.lineItems}</p> : null}</section>
