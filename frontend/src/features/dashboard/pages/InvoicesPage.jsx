...
const InvoicesPage = () => {
  ...
  const totals = useMemo(() => {
    const items = form.lineItems.map((item, index) => {
      ...
      const tax = gstPreview?.lines?.[index]?.totalTax ?? (taxable * Math.max(taxRate, 0)) / 100;
  ...
  const changeForm = (event) => setForm((current) => ({ ...current, [event.target.name]: event.target.value, ...(event.target.name === "customerId" ? { placeOfSupplyCode: customers.find(customer => customer._id === event.target.value)?.placeOfSupplyCode || customers.find(customer => customer._id === event.target.value)?.stateCode || "" } : {}) }));
  ...
  const resetEditor = () => { pendingIssue.current = null; issuePaymentKey.current = makePaymentForm().idempotencyKey; setEditingId(""); setForm(makeForm()); setErrors({}); setShowEditor(false); };
  const edit = (invoice) => {
