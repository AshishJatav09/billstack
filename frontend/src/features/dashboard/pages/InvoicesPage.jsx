...
const InvoicesPage = () => {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-8">
    ...
    {showEditor ? <form id="invoice-editor" onSubmit={(event) => save(event)} className="rounded-2xl border p-4 sm:p-5" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="flex flex-wrap items-start justify-between gap-4 border-b pb-5" style={{ borderColor: "var(--panel-border)" }}><div><p className="text-sm font-medium text-brand-600 dark:text-brand-300">{editingId ? "Editing invoice" : "New invoice"}</p><h3 className="mt-1 text-xl font-semibold">{editingId ? "Update invoice details" : "Create an invoice"}</h3><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>Check the details below, then issue your invoice.</p></div><button type="button" onClick={resetEditor} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm" style={{ borderColor: "var(--panel-border)" }}><X size={16} /> Close</button></div>
      {gstPreview && <div className="mt-3 rounded-xl border p-3 text-sm">Tax treatment: {gstPreview.supplierStateCode === gstPreview.placeOfSupplyCode ? "Intra-state" : "Inter-state"} · CGST {money(gstPreview.cgst)} · SGST {money(gstPreview.sgst)} · IGST {money(gstPreview.igst)}</div>}{gstPreviewError ? <div role="status" className="mt-2 flex items-start justify-between gap-3 text-sm text-rose-600"><span>{gstPreviewError}</span><button type="button" onClick={() => setGstPreviewError("")} className="shrink-0 rounded-lg p-1 hover:bg-rose-500/10" aria-label="Dismiss GST preview error"><X size={15} /></button></div> : null}
