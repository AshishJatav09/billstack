...
const InvoiceDetailPage = () => {
  ...
  const hasConfiguredCommunications = Boolean(providerStatus?.email?.configured || providerStatus?.whatsapp?.configured);
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-8">
    ...
    <div className={`grid gap-6 ${isRealEstateSelfHosted ? "" : "xl:grid-cols-[minmax(0,1fr)_330px]"}`}>
      ...
      <aside className={isRealEstateSelfHosted ? "grid items-start gap-6 md:grid-cols-2 xl:grid-cols-3" : "space-y-6"}>
        ...
        {(!isRealEstateSelfHosted || hasConfiguredCommunications) ? <CommunicationPanel deliveries={deliveries} reminders={reminders} templates={templates} form={communicationForm} setForm={setCommunicationForm} providerStatus={providerStatus} active={active} onSend={sendViaChannel} onSchedule={scheduleReminder} /> : null}
        <PaymentSummary invoice={invoice} paymentLabel={paymentLabel} paymentClass={paymentClass} />
        {(!isRealEstateSelfHosted || allocations.length) ? <PaymentAllocations invoice={invoice} rows={allocations} /> : null}
